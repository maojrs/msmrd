...
def partitionHalfSphere(num_partitions):
    """
    Calculates equal area partition of half unit sphere
    with "num_partitions" partitions.
    """

    # Functions to obtain angle from cap area and viceversa
    def angle_to_cap_area(phi):
        return 2*np.pi*(np.sin(phi/2.0))**2

    def cap_area_to_angle(area):
        return 2.0*np.arcsin(np.sqrt(area/(2.0*np.pi)))

    # Calculate areas of each state and polar caps angle (phi0 and pi-phi0)
    state_area = 2*np.pi/num_partitions
    phi0 = cap_area_to_angle(state_area)
    # Calculate the number of collars between the polar caps
    ideal_collar_angle = np.sqrt(state_area)
    ideal_num_collars = (np.pi - 2*phi0)/ideal_collar_angle
...
def plotPartitionedHalfSphere(numPartitions = None, save = None):
    """
    Plots partitioned half sphere function for a given numPartitions,
    save: set to True saves the output figure to file,
    """
    if numPartitions == None:
        numPartitions == 20
