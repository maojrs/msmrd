...
def simulationFPT(initialState, finalState, trajectorynum):
    '''
...
def multiprocessingHandler(initialState, finalState):
    '''
    ...
    simulationFPTpartial = partial(simulationFPT, initialState, finalState)
    with open(filename, 'w') as file:
        for index, result in enumerate(pool.imap(simulationFPTpartial, trajNumList)):
            initState, endState, time = result
            if initState != 'failed':
                file.write(str(initState) + ' ' + str(endState) + ' ' + str(time) + '\n')
                print("Simulation " + str(index) + " of transition " + str(initState) + ' to ' + str(endState) + ", done. Success!")
            else:
                print("Simulation " + str(index) + "of transition " + str(initState) + 'to' + str(endState) + ", done. Failed :(")
...
for initialState in boundStates:
	for finalState in boundStates:
		if (initialState != finalState):
			...
			# Run parallel code
			multiprocessingHandler(initialState, finalState)
